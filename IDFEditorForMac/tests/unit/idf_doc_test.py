...
class IdfDocumentTest(th.PyEplusTestCase):
    ...
    def test_read_returns_objects_for_valid_file(self):
        self.doc.readFromFile('test_file.idf')
    ...
    def test_writes_objects_read_from_file(self):
        ...
        self.doc.readFromFile('test_file.idf')
        self.doc.writeToFile('other_file.idf')
    ...
    def test_objects_for_class_returns_empty_list_when_no_object_for_class(self):
        self.assertEquals(self.doc.objectsOfClass('classNotFound'), [])

    def test_objects_for_class_leaves_other_objects_out(self):
        self.doc.readFromFile('test_file.idf')
        only_version = self.doc.objectsOfClass('Version')
        self.assertEquals(only_version[0][0], 'Version')
        only_schedule_type_limits = self.doc.objectsOfClass('ScheduleTypeLimits')
    ...
    def test_fields_of_class_returns_empty_list_when_class_not_found(self):
        self.assertEqual(self.doc.fieldsOfClass("classNotFound"), [])

    def test_fields_of_class(self):
        self.doc.readFromFile('test_file.idf')
        self.assertEquals(self.doc.fieldsOfClass('Version'), ['Version Identifier'])
        self.assertEquals(self.doc.fieldsOfClass('ScheduleTypeLimits'), ['Name', 'Lower Limit Value', 'Upper Limit Value', 'Numeric Type', 'Unit Type'])
        self.assertEquals(self.doc.fieldsOfClass('Timestep'), ['Number of Timesteps per Hour'])

    def test_new_object_returns_empty_object_with_class_name(self):
        self.doc.addEmptyObject('ScheduleTypeLimits')
        self.assertEquals(self.doc.objectsOfClass('ScheduleTypeLimits'), ['ScheduleTypeLimits,,,,,'.split(',')])

    def test_returns_object_of_class_at_index(self):
        self.doc.readFromFile('test_file.idf')
        self.assertEquals(self.doc.objectOfClassAtIndex('Version', 0), ['Version', '7.2'])
        self.assertEquals(self.doc.objectOfClassAtIndex('Version', 1), [])

    def test_replaces_object_at_index(self):
        self.doc.readFromFile('test_file.idf')
        self.doc.replaceObjectAtIndexWithObject(0, ['Version', '8.0'])
        self.assertEquals(self.doc.objectsOfClass('Version'), [['Version', '8.0']])

    def test_deletes_object_at_valid_index(self):
        self.doc.readFromFile('test_file.idf')
        self.doc.deleteObjectOfClassAtIndex('Version', 0)
        self.assertEquals(self.doc.objectsOfClass('Version'), [])

    def test_does_nothing_when_deleting_object_at_invalid_index(self):
        self.doc.readFromFile('test_file.idf')
        self.doc.deleteObjectOfClassAtIndex('Version', 1)
        self.assertEquals(self.doc.objectsOfClass('Version'), [['Version', '7.2']])
